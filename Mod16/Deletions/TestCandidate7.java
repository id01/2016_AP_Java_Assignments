...
// It now also uses replacements and insertions and delections.
...
public class TestCandidate7
{
	...
	public static Candidate[] deleteByLoc(Candidate [] election, int needle)
	{
		Candidate [] newElection = new Candidate[election.length-1];
		int i=0; boolean deleted = false;
		for ( Candidate person : election ) {
			if (i != needle || deleted == true) {
				newElection[i] = person;
				i++;
			} else {
				deleted = true; // I already deleted it. Move on.
			}
		}
		return newElection;
	}

	public static Candidate[] deleteByName(Candidate [] election, String needle)
	{
		Candidate [] newElection = new Candidate[election.length-1];
		int i=0;
		for ( Candidate person : election ) {
