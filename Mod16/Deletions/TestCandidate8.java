// Gavin Lo
// 04-28-2017
// This program tests the Candidate class using an ArrayList and traversals
// It now also uses replacements and insertions and deletions.
...
public class TestCandidate8
{
	...
	public static void deleteByLoc(ArrayList<Candidate> election, int needle)
	{
		election.remove(needle);
	}

	public static void deleteByName(ArrayList<Candidate> election, String needle)
	{
		int locationToRemove = 0;
		for (int i=0; i<election.size(); i++) {
			if ( election.get(i).getName().equalsIgnoreCase(needle) )
				locationToRemove = i;
		}
		election.remove(locationToRemove);
	}
